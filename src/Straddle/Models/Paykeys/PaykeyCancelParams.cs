...
/// </summary>
public record class PaykeyCancelParams : ParamsBase
{
    ...
    public PaykeyCancelParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public PaykeyCancelParams(PaykeyCancelParams paykeyCancelParams)
        : base(paykeyCancelParams)
    {
        this.ID = paykeyCancelParams.ID;

        this._rawBodyData = new(paykeyCancelParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static PaykeyCancelParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(PaykeyCancelParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/paykeys/{0}/cancel", this.ID)
        )
        {
