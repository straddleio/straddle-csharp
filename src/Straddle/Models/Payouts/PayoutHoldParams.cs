...
/// </summary>
public record class PayoutHoldParams : ParamsBase
{
    ...
    public required string Reason
    {
        get
        {
            ...
            return this._rawBodyData.GetNotNullClass<string>("reason");
        }
    ...
    public PayoutHoldParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public PayoutHoldParams(PayoutHoldParams payoutHoldParams)
        : base(payoutHoldParams)
    {
        this.ID = payoutHoldParams.ID;

        this._rawBodyData = new(payoutHoldParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static PayoutHoldParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(PayoutHoldParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/v1/payouts/{0}/hold", this.ID)
        )
        {
