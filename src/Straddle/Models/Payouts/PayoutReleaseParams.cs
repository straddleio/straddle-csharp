...
/// </summary>
public record class PayoutReleaseParams : ParamsBase
{
    ...
    public PayoutReleaseParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public PayoutReleaseParams(PayoutReleaseParams payoutReleaseParams)
        : base(payoutReleaseParams)
    {
        this.ID = payoutReleaseParams.ID;

        this._rawBodyData = new(payoutReleaseParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static PayoutReleaseParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(PayoutReleaseParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/payouts/{0}/release", this.ID)
        )
        {
