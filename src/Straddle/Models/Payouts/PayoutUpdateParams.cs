...
/// </summary>
public record class PayoutUpdateParams : ParamsBase
{
    ...
    public PayoutUpdateParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public PayoutUpdateParams(PayoutUpdateParams payoutUpdateParams)
        : base(payoutUpdateParams)
    {
        this.ID = payoutUpdateParams.ID;

        this._rawBodyData = new(payoutUpdateParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static PayoutUpdateParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(PayoutUpdateParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/v1/payouts/{0}", this.ID)
        )
        {
