...
/// </summary>
public record class PayoutCancelParams : ParamsBase
{
    ...
    public PayoutCancelParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public PayoutCancelParams(PayoutCancelParams payoutCancelParams)
        : base(payoutCancelParams)
    {
        this.ID = payoutCancelParams.ID;

        this._rawBodyData = new(payoutCancelParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static PayoutCancelParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(PayoutCancelParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/payouts/{0}/cancel", this.ID)
        )
        {
