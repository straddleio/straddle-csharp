...
/// </summary>
public record class ChargeUpdateParams : ParamsBase
{
    ...
    public ChargeUpdateParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public ChargeUpdateParams(ChargeUpdateParams chargeUpdateParams)
        : base(chargeUpdateParams)
    {
        this.ID = chargeUpdateParams.ID;

        this._rawBodyData = new(chargeUpdateParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static ChargeUpdateParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(ChargeUpdateParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/v1/charges/{0}", this.ID)
        )
        {
